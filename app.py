...
# v37 - Base de datos con ruta absoluta para evitar limpiezas de archivos temporales
DB_PATH = os.path.join(os.getcwd(), 'ordoklar_v37_data.db')

def init_db():
    """Inicialización única y robusta de la base de datos."""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    # Usamos IF NOT EXISTS para proteger los datos existentes
    cursor.execute('''
...
# Inicializar al arrancar el script
init_db()
...
# --- APIs CRUD (v37 con manejo de errores mejorado) ---
    ...
    try:
        if request.method == 'POST':
            ...
            conn.execute("UPDATE personal SET nombre=?, apellido=?, legajo=? WHERE id=?", 
                         (d['nombre'], d['apellido'], d['legajo'], d['id']))
            conn.commit()
...
def handle_puestos():
    ...
    try:
        if request.method == 'POST':
            ...
            d = request.json
            conn.execute("UPDATE puestos SET nombre=?, horario=?, cantidad=? WHERE id=?", 
                         (d['nombre'], d['horario'], d['cantidad'], d['id']))
            conn.commit()
        elif request.method == 'DELETE':
...
# --- INTERFAZ v37 (Optimización de renderizado y estabilidad) ---
HTML_UI = '''
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <title>ORDO KLAR | Gestión Técnica v37</title>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js"></script>
    <style>
        :root { --gold: #D4AF37; --bg: #000; --card: #111; --border: #222; }
        body { background: var(--bg); color: #E0E0E0; font-family: 'Segoe UI', sans-serif; margin: 0; overflow-x: hidden; }
        
        .header-main { text-align: center; padding: 20px; border-bottom: 2px solid var(--gold); background: #050505; }
        .brand-logo { font-size: 28px; font-weight: 900; letter-spacing: 6px; margin: 0; }
        ...
        /* Planilla v37 - Full Width No Scroll */
        .table-wrap { width: 100%; border-radius: 8px; border: 1px solid var(--border); background: #000; }
        table { width: 100%; border-collapse: collapse; table-layout: fixed; font-size: 10px; }
        th, td { border: 1px solid #222; text-align: center; padding: 5px 1px; }
        
        .name-col { text-align: left; width: 140px; padding-left: 8px; font-weight: bold; color: var(--gold); background: #080808; border-right: 2px solid var(--gold); font-size: 11px; }
        
        .th-date { height: 80px; vertical-align: bottom; }
        .date-label { transform: rotate(-90deg); display: block; white-space: nowrap; font-weight: bold; margin-bottom: 8px; }
        .date-label b { color: var(--gold); font-size: 12px; }
        ...
        .row-total { background: #0a0a0a; color: var(--gold); font-weight: 900; font-size: 11px; }

        /* Boxes de Puestos */
        .puestos-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 15px; }
        .puesto-box { background: var(--card); border: 1px solid var(--border); border-radius: 12px; padding: 15px; border-left: 4px solid var(--gold); position: relative; }
        .puesto-box h3 { margin: 0 0 10px 0; font-size: 16px; color: var(--gold); text-transform: uppercase; }
        ...
        input, select { background: #000; border: 1px solid var(--border); color: #fff; padding: 12px; border-radius: 6px; font-size: 14px; }
        input:focus { border-color: var(--gold); outline: none; }
        .btn-gold { background: var(--gold); color: #000; border: none; padding: 12px; font-weight: 900; border-radius: 6px; cursor: pointer; text-transform: uppercase; }
    </style>
</head>
<body>
    ...
    <header class="header-main no-print">
        ...
        <div style="font-size: 10px; letter-spacing: 3px; color: #666; margin-top: 5px;">SISTEMA DE GESTIÓN TANDANOR</div>
    </header>
    ...
    <div class="content">
        ...
        <div id="s-pla" class="section active-section">
            <div class="form-box no-print">
                ...
                <div style="color:var(--gold); font-size:11px; align-self:center; text-align:right">Modo: Clic para rotar estado</div>
            </div>
        ...
        <div id="s-pue" class="section">
            <div class="form-box no-print">
                ...
                <input type="text" id="pue-hor" placeholder="Rango Horario">
        ...
        <div id="s-per" class="section">
            <div class="form-box no-print">
                ...
                <input type="text" id="per-leg" placeholder="N° Legajo">
            ...
            <div class="table-wrap" id="render-personal">
                <table style="font-size: 13px;">
                    <thead><tr><th style="width:100px">LEGAJO</th><th style="text-align:left">APELLIDO Y NOMBRE</th><th>ACCIONES</th></tr></thead>
        ...
        <div id="s-inf" class="section">
            <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 20px;">
                <div style="background:var(--card); padding:30px; border-radius:15px; text-align:center; border:1px solid var(--border)">
                    <h3>IMPRIMIR PLANILLA</h3>
                    <button class="btn-gold" onclick="genPDF('render-planilla', 'Planilla_Mensual', 'a3', 'landscape')">Descargar A3</button>
                </div>
                <div style="background:var(--card); padding:30px; border-radius:15px; text-align:center; border:1px solid var(--border)">
                    <h3>NÓMINA PERSONAL</h3>
                    <button class="btn-gold" onclick="genPDF('render-personal', 'Nomina_Personal', 'a4', 'portrait')">Descargar A4</button>
                </div>
                <div style="background:var(--card); padding:30px; border-radius:15px; text-align:center; border:1px solid var(--border)">
                    <h3>REPORTE PUESTOS</h3>
                    <button class="btn-gold" onclick="genPDF('grid-puestos', 'Reporte_Puestos', 'a4', 'portrait')">Descargar A4</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
            ...
            try {
                ...
                document.getElementById('grid-puestos').innerHTML = pue.map(x => `
                    <div class="puesto-box">
                        <h3>${x.nombre}</h3>
                        <p style="font-size:12px; color:#999">${x.horario} | Dotación: ${x.cantidad}</p>
                        <div class="no-print">
                            <button onclick="editPue(${x.id},'${x.nombre}','${x.horario}',${x.cantidad})" style="background:#1976D2; border:none; color:#fff; padding:4px 8px; border-radius:4px; cursor:pointer">Editar</button>
                            <button onclick="delPue(${x.id})" style="background:#D32F2F; border:none; color:#fff; padding:4px 8px; border-radius:4px; cursor:pointer">Borrar</button>
                        </div>
                    </div>
                `).join('');
                ...
                document.getElementById('b-per').innerHTML = per.map(x => `
                    <tr><td>${x.legajo}</td><td style="text-align:left">${x.apellido.toUpperCase()}, ${x.nombre}</td><td class="no-print">
                        <button onclick="editPer(${x.id},'${x.legajo}','${x.apellido}','${x.nombre}')" style="background:#1976D2; border:none; color:#fff; padding:4px 8px; border-radius:4px; cursor:pointer">Editar</button>
                        <button onclick="delPer(${x.id})" style="background:#D32F2F; border:none; color:#fff; padding:4px 8px; border-radius:4px; cursor:pointer">Borrar</button>
                    </td></tr>
                `).join('');
        ...
        async function cycleEstado(td, pid, fecha) {
            ...
            const res = await fetch('/api/novedades', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({p_id: pid, fecha: fecha, estado: next})});
            if (res.ok) render();
        }
