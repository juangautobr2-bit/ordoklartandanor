...
app = Flask(__name__)
# Usamos una ruta absoluta para evitar problemas de permisos en algunos servidores
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DB_PATH = os.path.join(BASE_DIR, 'ordoklar.db')
...
<head>
    ...
    <style>
        ...
        .t-planilla th, .t-planilla td { border: 1px solid #222; text-align: center; padding: 5px; }

        .col-personal { width: 180px; text-align: left !important; padding-left: 10px !important; color: var(--gold); background: #080808; }
        .sel-planilla { width: 100%; background: transparent; color: white; border: none; cursor: pointer; font-weight: bold; }
        
        input, select { background: #000; border: 1px solid #333; color: #fff; padding: 10px; border-radius: 6px; }
        .btn { border-radius: 6px; padding: 10px 20px; cursor: pointer; font-weight: 700; border: none; }
        .btn-gold { background: var(--gold); color: #000; }
        .btn-outline { background: transparent; border: 1px solid #444; color: var(--text-dim); }

        .cell-12 { background-color: var(--color-12); }
        .cell-F { background-color: var(--color-F); color: #000 !important; }
...
<script>
    ...
    function renderPlanilla() {
        ...
        let h1 = `<tr><th class="col-personal">PERSONAL</th><th>LEG</th>`;
        for(let i=1; i<=totalDias; i++) h1 += `<th>${i}</th>`;
        h1 += `</tr>`;
        document.getElementById('h-mensual').innerHTML = h1;
        ...
        document.getElementById('b-mensual').innerHTML = personal.filter(p => p.estado_p === 'ACTIVO').map(p => {
            ...
            return `<tr><td class="col-personal">${p.apellido.toUpperCase()}</td><td>${p.legajo}</td>${celdas}</tr>`;
        }).join('');
    }
    ...
    async function savePersonal() {
        ...
        const url = editId ? `/api/personal/${editId}` : '/api/personal';
        
        const res = await fetch(url, { method, headers: {'Content-Type':'application/json'}, body: JSON.stringify(d) });
        if(!res.ok) { alert("Error: Legajo duplicado o datos inválidos"); return; }
    ...
    function renderPersonal() {
        document.getElementById('lista-personal').innerHTML = personal.map(p => `
            <tr style="background:var(--light-gray); border-bottom:2px solid black;">
                <td style="padding:15px; color:var(--gold); font-weight:bold;">${p.legajo}</td>
                <td>${p.apellido.toUpperCase()}, ${p.nombre}</td>
                <td><label class="switch"><input type="checkbox" ${p.estado_p==='ACTIVO'?'checked':''} onchange="toggleStatus(${p.id}, '${p.estado_p}')"><span class="slider"></span></label></td>
                <td><button onclick="deletePersonal(${p.id})">ELIMINAR</button></td>
            </tr>`).join('');
    }
    ...
    async function deletePersonal(id) { if(confirm("¿Eliminar?")) { await fetch(`/api/personal/${id}`, {method:'DELETE'}); loadData(); } }

    function renderPuestos() {
        document.getElementById('grid-puestos').innerHTML = puestos.map(pst => `
            <div class="card">
                <b style="color:var(--gold)">${pst.nombre.toUpperCase()}</b>
                <p>Plazas: ${pst.cantidad}</p>
            </div>`).join('');
    }
