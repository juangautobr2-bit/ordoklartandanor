...
app = Flask(__name__)
app.secret_key = 'ordo_klar_v59_key'

# Base de Datos
DB_PATH = os.path.abspath("ordoklar_v59.db")
USER_ADMIN = "admin"
PASS_ADMIN = "admin123"
...
<html lang="es">
<head>
    <meta charset="UTF-8"><title>ORDO KLAR v59</title>
    <style>
        ...
        <div id="s-pue" class="section">
            <div class="box no-print" style="display:flex; flex-wrap:wrap; gap:10px; align-items:center">
                ...
                <div style="border-left:1px solid #333; padding-left:15px; margin-left:15px; display:flex; gap:10px">
                    <button class="btn" style="background:#1b4332; color:white" onclick="toggleGlobalFix(true)">🔒 FIJAR TODAS LAS GUARDIAS</button>
        ...
        <div id="s-per" class="section">
            ...
            <div class="box">
                <table style="width:100%; text-align:left">
                    <thead style="color:var(--gold)"><tr><th>Legajo</th><th>Apellido</th><th>Nombre</th><th>Acción</th></tr></thead>
        ...
        async function render() {
            ...
            document.getElementById('grid-pue').innerHTML = pue.map(x => {
                ...
                <div class="card-pue" id="pue-${x.id}">
                    ...
                    <h3>${x.nombre}</h3><small>${x.horario}</small>${s}
                </div>`;
            }).join('');
        ...
        function toggleGlobalFix(fix) {
            ...
            if(fix) {
                ...
                document.querySelectorAll('.slot').forEach(slot => {
                    ...
                    const span = slot.querySelector('.slot-fixed');
                    span.innerText = sel.value || "---";
                });
            } else {
                grid.classList.remove('global-fixed');
            }
        }
