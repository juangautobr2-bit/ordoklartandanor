...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'ordo_klar_v75_stable')

# --- CONEXIÓN A NEON (POSTGRESQL) ---
def get_db_connection():
    # Recordá configurar DATABASE_URL en el panel de Environment de Render
...
def init_db():
    ...
    cur.execute('CREATE TABLE IF NOT EXISTS novedades (personal_id INTEGER, fecha TEXT, estado TEXT, UNIQUE(personal_id, fecha))')
    conn.commit()
    cur.close()
    conn.close()
...
# --- API: PUESTOS Y ASIGNACIONES (BASE ORIGINAL) ---
...
def asignar_batch():
    ...
    for a in d['asignaciones']:
        if a['per_id']:
            ...
            cur.execute("DELETE FROM asignaciones WHERE puesto_id=%s AND slot_index=%s", (d['puesto_id'], a['slot']))
    conn.commit()
    cur.close()
    conn.close()
